...
```{bash, eval = FALSE}
cd Documents%R%MY472
git clone https://github.com/keis478/lse-my472.github.io.git
cd seminar2
```
...
the sample?  What are the lowest values for monthly percentage changes of 
industrial production for the two countries? Make sure to delete NA values in 
only the time series of interest. (_Optional_: can you create a function that 
would do this for any country?)
...
ipu_clean <- ip_and_unemployment %>%
  pivot_wider(id_cols = c("country", "date"), #remove the columns not needed
...
ipu_clean %>%
  group_by(country) %>%
  group_by(nonNA_ip = sum(!is.na(ip)),
...
ipu_clean %>%
  ...
  group_by(nonNA_ip = sum(!is.na(ip)),
            ...
            nonNA_ue_pct = nonNA_ue / unemployment)
...
```{bash, eval = FALSE}
git add 03-exercises-tabular-data.Rmd
git commin -m "finished first half"
git push
```
...
ipu_clean %>%
  mutate(ip_sq = ip ^ 2,
         ...
         ip_unemployment = unemployment + ip ) %>%
  head()
